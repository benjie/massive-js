...
DB.prototype.withConnectionWrapper = function(wrapper) {
  var oldWrapper = this.connectionWrapper;
  var clone = _.clone(this);
  clone.query = null;
  clone.connectionWrapper = function (fn) {
    var oldHandler = oldWrapper(fn);
    return function(err, db, done) {
      if (err) return oldHandler(err, db, done);
      wrapper(db, oldHandler, done);
    };
  }
  return clone;
};
